/**
 * When reloading the page, fetch the corresponding data from server database 
 * according to the URL query projectId and show on the screen. 
...
/**
 * Dropdown menu event listener on top nav bar, when user selecting another project 
 * to view, it'll be triggered and automatically update the query for projectId in URL 
 * and also re-render the data showing on task page based on the new selected projectId.
 * Generated by ChatGPT 3.5 and partial code by Joyce Huang
 *
 * @author https://chat.openai.com/
 * @author Joyce Huang
...
/**
 * Functions below fomatDate, formatTime, combineDateTime are used to combine date and time 
 * from task form and format it into the pattern that luxon required to do the time zone conversion. 
 * async function convertToUserTimezone is the actual function that perform the time zone conversion using luxon library.
 * Generated by ChatGPT 3.5 and partial code by Joyce Huang
 *
 * @author https://chat.openai.com/
 * @author Joyce Huang
 */
// format the time as 12 hour with am and pm
        ...
        for (const todo of tasksData) {
            ...
            if (
                ...
                try {
                    ...
                    taskTag += `<div class="task-card" id="task-item-${todo._id}" taskcard-id="${todo._id}">
 ...
 * 
 * 
 * Generated by ChatGPT 3. and partial code by Joyce Huang
 *
 * @author https://chat.openai.com/
 * @author Joyce Huang
 */
 ...
 * @author Joyce Huang
 */
async function convertTaskDetailToUserTimezone(taskdetail) {
    try {
        const response = await fetch('/getCurrentUserId');
        const data = await response.json();
        const userId = data.userId;

        const member = taskdetail.taskMember.find(member => member._id.toString() === userId);
        if (!member) {
            throw new Error('Member not found in taskMembers');
        }
        ...
        if (memberTimezone === taskOwnerTimezone) {
            console.log("No timezone conversion needed.");
            console.log("no convert:", taskdetail.dueDate);
            return {
        ...
        // Convert start date time to user timezone
        const startDateTime = DateTime.fromISO(combineDateTime(taskdetail.startDate, taskdetail.startTime), { zone: taskOwnerTimezone }).setZone(memberTimezone);
        // Convert due date time to user timezone
        const dueDateTime = DateTime.fromISO(combineDateTime(taskdetail.dueDate, taskdetail.dueTime), { zone: taskOwnerTimezone }).setZone(memberTimezone);

        // format the date, time to be accepted by form 
        const formattedStartDate = startDateTime.toFormat('yyyy-MM-dd');
        const formattedDueDate = dueDateTime.toFormat('yyyy-MM-dd');
        const formattedStartTime = startDateTime.toFormat('HH:mm');
        const formattedDueTime = dueDateTime.toFormat('HH:mm');
...
/**
 * HERE IS THE PART THAT SHOW TASK DETAILS
 * 
 * Generated by ChatGPT 3.5 and partial code by Joyce Huang
 *
 * @author https://chat.openai.com/
...
document.addEventListener('click', async function(e){
    if (e.target.closest('.task-card')){
        ...
        const taskId = taskCard.getAttribute('taskcard-id');
        
        // Check if the clicked element or its parent is `.checkbox-wrapper`
        if (!e.target.closest('.checkbox-wrapper')) {
            try{
                const response = await fetch(`/getTaskcardDetails/${taskId}`);
                if(response.ok){
                    const taskData = await response.json();

                    // convert to current user's time zone to show in task detail modal
                    const taskDataTimezone = await convertTaskDetailToUserTimezone(taskData);

                    // put the retrieved data into modal 
                    const taskModalContent = document.getElementById('taskdetailModalBody');
                    const completedMembersLength = taskData.completedMembers.length;
                    const taskMembersLength = taskData.taskMember.length;
                    const completionRatio = Math.round((completedMembersLength / taskMembersLength) * 100);

                    const taskModalHTML = `
                        <form id="taskForm">
                            <div class="form-group">
                                <label for="taskModalTitle">Title</label>
                                <input type="text" class="form-control" id="taskModalTitle" value="${taskData.title}" disabled>
                            </div>
                            <div class="form-group">
                                <label for="taskModalDescription">Description</label>
                                <input type="text" class="form-control" id="taskModalDescription" value="${taskData.description}" disabled>
                            </div>
                            <div class="form-group">
                                <label for="taskModalStartDate">Start Date</label>
                                <input type="date" class="form-control" id="taskModalStartDate" value="${taskDataTimezone.startDate}" disabled>
                            </div>
                            <div class="form-group">
                                <label for="taskModalStartTime">Start Time</label>
                                <input type="time" class="form-control" id="taskModalStartTime" value="${taskDataTimezone.startTime}" disabled>
                            </div>
                            <div class="form-group">
                                <label for="taskModalDueDate">Due Date</label>
                                <input type="date" class="form-control" id="taskModalDueDate" value="${taskDataTimezone.dueDate}" disabled>
                            </div>
                            <div class="form-group">
                                <label for="taskModalDueTime">Due Time</label>
                                <input type="time" class="form-control" id="taskModalDueTime" value="${taskDataTimezone.dueTime}" disabled>
                            </div>
                            <div class="form-group">
                                <label>Task Created By: </label> 
                                <span class="task-modal-username">${taskData.taskOwner}</span>
                            </div>
                            <div class="form-group">
                                <label>Status: </label> 
                            ...
                            <div class="form-group" id="members">
                                ...
                                <!-- Here you can dynamically append member inputs -->
                            </div>
                        </form>
                    `;

                    let membersHTML = '';
                    const membersData = taskData.taskMember;
                    membersData.forEach(member => {
                        const username = member.username;
                        const email = member.email;
                        membersHTML += `
                            <div>
                                <span  class="task-modal-username">${username}</span> <span>(${email})</span>
                            </div>
                        `;
                    });

                    taskModalContent.innerHTML = taskModalHTML;

                    const membersContainer = document.getElementById('members');
                    membersContainer.innerHTML = '<label for="members">Members:</label>' + membersHTML;
            ...
            var taskModalInstance = new bootstrap.Modal(taskModal);
